      ...
      <div className="bg-surface-container-highest rounded-3xl p-12 lg:p-16 border border-outline-variant/50 shadow-sm relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 max-w-7xl mx-auto relative z-10">
          ...
          <div className="col-span-1 md:col-span-2 md:col-start-6 space-y-6">
            ...
            <ul className="space-y-4">
              ...
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.COLLECTIONS}>Collections</Link></li>
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.TEMPLATES}>Templates</Link></li>
            </ul>
          </div>
          <div className="col-span-1 md:col-span-2 space-y-6">
            <h5 className="font-label-md text-label-md uppercase tracking-widest text-on-surface">Company</h5>
            <ul className="space-y-4">
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.TERMS}>Terms</Link></li>
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.PRIVACY}>Privacy</Link></li>
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.SUPPORT}>Support</Link></li>
              <li><Link className="text-on-surface-variant font-body-md hover:text-secondary transition-colors duration-300" to={PATHS.CONTACT}>Contact</Link></li>
            </ul>
          </div>
        ...
        <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-outline-variant/30 flex flex-col md:flex-row justify-between items-center gap-4 relative z-10">
          ...
          <div className="flex gap-8 text-label-sm font-label-sm text-on-surface-variant">
            <Link to={PATHS.SHIPPING} className="hover:text-secondary transition-colors">Shipping Policy</Link>
            <Link to={PATHS.REFUNDS} className="hover:text-secondary transition-colors">Refunds</Link>
          </div>
        </div>
      </div>
    </footer>
